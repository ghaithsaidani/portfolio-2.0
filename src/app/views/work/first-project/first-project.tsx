...
export function FirstProject(props: FirstProjectProps) {
  ...
  return (
    <article
      ...
      <div style={{
      ...
      }} className={'flex flex-col items-center lg:items-start xl:w-[32%] 2xl:w-[28%] mt-10 lg:mt-44 z-20'}>
        <Badge variant={'secondary'}>web application</Badge>
        <h1
          className="text-[2rem] sm:text-[2.5rem] lg:text-[3rem] mt-4 leading-none font-sfProBold text-black dark:text-white text-nowrap">
          Personal Portfolio
        </h1>
        <p
          className={'font-sfPro text-gray-400 mt-4 text-xs sm:text-sm lg:text-base text-center px-9 lg:px-0 lg:text-left'}>My
        ...
        <a className={'mt-5 flex items-center gap-2'} href={'https://ghaith-saidani.netlify.app/'} target={'_blank'}
          ...
          <MovingBorderButton
            ...
            className="bg-background text-xs sm:text-sm border-[1px] border-border hover:text-primary">
      ...
      <div
        ...
        className={'border-2 border-border place-self-center lg:place-self-end h-[70%] w-[65%] lg:h-[95%] lg:w-[22%] p-[10px] sm:p-[15px] bg-card rounded-[30px] sm:rounded-[50px] relative flex justify-center'}>
        <div className={'rounded-[20px] sm:rounded-[35px] w-full overflow-hidden'}>
          <img src={OldPortfolio} alt={'old portfolio'} className={'object-fill h-full w-full'} />
        </div>
        <div className={'absolute h-5 w-20 sm:h-6 sm:w-28 bg-card top-2 sm:top-3.5 rounded-b-3xl'}></div>
      </div>
