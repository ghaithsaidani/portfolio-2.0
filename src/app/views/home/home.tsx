...
export function Home(props: HomeProps) {
  const socialMedias: IconInterface[] = [
    {
      name: 'LinkedIn',
      icon: <FaLinkedinIn className={'w-4 h-4 lg:h-5 lg:w-5'}/>,
      color: 'linkedin',
      link: 'https://www.linkedin.com/in/ghaith-saidani-9230571b2/'
    },
    {
      name: 'Github',
      icon: <FaGithub className={'w-4 h-4 lg:h-5 lg:w-5'}/>,
      color: 'github',
      link: 'https://github.com/ghaithsaidani'
    },
    {
      name: 'LeetCode',
      icon: <TbBrandLeetcode className={'w-4 h-4 lg:h-5 lg:w-5'}/>,
      color: 'leetCode',
  ...
  return (
    <section className={'home lg:h-screen'}>
      <article
        className={'flex flex-col-reverse justify-between lg:flex-row py-20 xl:px-14 2xl:px-0 h-screen lg:justify-center items-center 2xl:gap-20 '}>
        <div className={'description flex flex-col items-center lg:items-start w-3/4 xl:w-3/5 2xl:w-2/5'}>
          <h1 className={'font-sfPro text-[1.2rem] lg:text-[1.5rem] 2xl:text-[2rem] flex flex-col items-center lg:items-start'}>
            <Reveal><span className={'text-2xl lg:text-4xl xl:text-6xl 2xl:text-[4rem] font-poetsenOne'}>Hey, I'm Ghaith<span
          ...
          <Reveal><p className={'font-sfPro text-gray-400 text-[0.85rem] lg:pr-52 mt-3 text-center lg:text-left'}>
            ...
            <div className={'flex gap-5 mt-12'}>
              <Link offset={-80} to={'contacts'} smooth={true} duration={500} className={'flex items-center gap-2'}>
                <MovingBorderButton
                  ...
                  className="bg-primary hover:text-primary hover:bg-background border-[1px] border-transparent hover:border-border transition-all duration-500 text-white"
                >
                  Say Hi &#x1F44B;
              ...
              <Link offset={-80} to={'work'} smooth={true} duration={500} className={'flex items-center gap-2'}>
                <MovingBorderButton
                  borderRadius="1.75rem"
                  containerClassName={'w-44 2xl:w-52'}
                  className=" bg-background  hover:text-primary border-[1px] border-border"
        ...
        <aside className={'h-[35%] md:h-[45%] lg:h-[60%] 2xl:h-[70%] flex gap-5 lg:gap-14 2xl:gap-20'}>
          <div className={'h-full relative'}>
            ...
            <div
              className={
                'absolute w-full h-[90%] bg-primary -z-10 bottom-0 rounded-xl before:w-full before:h-full before:absolute before:border-[3px] before:border-black dark:before:border-white before:top-10 before:right-20 before:rounded-xl'
              }
            ></div>
          </div>
          <div
          ...
          >
            <div
              className={'font-sfPro text-[0.7rem] lg:text-[0.85rem] 2xl:text-[1rem] flex text-primary text-nowrap'}
              style={{ writingMode: 'vertical-rl' }}
            >
              Follow me on:
            </div>
            <Separator orientation={'vertical'} className={'h-20 md:h-32 bg-primary'} />
            <ul className={'grid gap-y-4'}>
              {socialMedias.map((socialMedia) => (
                <li
                  key={socialMedia.name}
                  className={`flex group relative bg-transparent border-[1px] border-primary text-primary hover:border-transparent social cursor-pointer items-center justify-center h-8 w-8 xl:h-10 xl:w-10 2xl:h-[120%] 2xl:w-[120%] rounded-full p-2 transition-colors duration-500  hover:bg-transparent hover:text-white overflow-hidden`}
                >
                  <a
